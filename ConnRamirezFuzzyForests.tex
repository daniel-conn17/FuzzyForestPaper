...
Fuzzy forests uses Weighted Gene Coexpression Network Analysis (WGCNA) to detect groups of highly correlated features.  Unbiased rankings are
obtained by fitting separate random forests on each module.  We also introduce our implementation of fuzzy forests in the \proglang{R} package, \pkg{fuzzyforest}.}
...
In this paper we have presented the fuzzy forests algorithm as an extension of random forests that can provide less biased feature selection in 
 ...
 The \pkg{fuzzyforest} package has two function for fitting the fuzzy forests algorithm.  The first implementation, \code{WGCNA_fuzzyforest} automatically 
