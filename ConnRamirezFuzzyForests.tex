...
%% almost as usual
\author{Daniel Conn\\UCLA SPH, Biostatistics \And Tuck Ngun \And  Gang Li\\UCLA SPH, Biostatistics \And Christina Ramirez\\UCLA SPH, Biostatistics}
...
In section 3, we introduce the \proglang{R} package \pkg{fuzzyforest}.  In section 4, we conduct simulations to comparing fuzzy forests to both random forests and conditional inference forests (CIFs).  We demonstrate that fuzzy forests has performance comparable to that of CIFs' although at much lower computational cost.  In section 5, we use fuzzy forests to determine which biological factors are important in determining how well an HIV patient copes with the virus.  Section 6 ends the article with a discussion and summary of our results. 
...
We generate data using the function 
\code{simulateDatExpr5Modules} and demonstrate the usual application of \newline \code{WGCNA_fuzzyforest}.
\begin{CodeChunk}
\begin{CodeInput}
R> n <- 1000
R> p <- 2000
R> MEs <- matrix(rnorm(nGenes*n), nrow=n, ncol=nGenes) 
R> sim <- simulateDatExpr5Modules(nGenes=p, simulateProportions=rep(1/5,5),  
+  MEt=MEs[, 1], MEbl=MEs[,2], MEbr=MEs[,3], MEy=MEs[,4], MEgr=MEs[,5])
R> X <- simMods$datExpr
R> y <- X%*%c(rep(2, 3),rep(0, p - 3)) + rnorm(n)
R> fit <- WGCNA_fuzzyforest(X, y, WGCNA_params = WGCNA_control(p=6),
+ screen_params = screen_control(mtry_factor=2),
+ select_params = select_control(number_selected=2), num_processors=1)
\end{CodeInput}
\end{CodeChunk}
\code{WGCNA_fuzzyforest} returns an \code{S3} object of type \code{fuzzy_forest} consisting of a list.  This list 
contains a \code{data.frame} with the ranking and VIM of the selected features.  It also contains the final random
...
(Insert section on plot function here)
\section{Simulations}
